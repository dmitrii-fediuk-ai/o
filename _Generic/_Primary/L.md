...
## Examples
`A ᗉ ⠿ ⟨ ball ⟩` означает, что `A` — это «a ball».
## Следствие
⊤ ⟨ `A` ∈ `S` ⟩ 
...
~~~
## Meaning
Такая запись вводит понятие `A` как «определённый, конкретный элемент `S`, о котором далее будет идти речь в запросе».
###
`S` — это `𝕊` / `𝒞` / `ℂ` / `𝕋`.
###
Сам по себе синтаксис `A ⦿ S` не содержит конкретики, какой именно элемент `A` имеется в виду.
Эту конкретику надо искать в дальнейшей части запроса.
###
По смыслу `⦿` соответствует определённому артиклю «the» английского языка.
## Example
`ꆜ ⦿ ⠿ ⟨ клиент ⟩` означает, что `ꆜ` — это не неопределённый клиент, а некий конкретный клиент, о котором дальше будет идти речь в запросе.
Конкретная информация о `ꆜ` будет дана в дальнейшей части запроса.
## Следствие
⊤ ⟨ `A` ∈ `S` ⟩ 
