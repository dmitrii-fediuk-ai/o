...
## Meaning
- Правило `A ≔ B` действует временно: только в рамках `☷`.
- `A` — это ⟨ Ⱳ https://en.wikipedia.org/wiki/Local_variable ⟩ 
- `☷` — это ⟨ Ⱳ https://en.wikipedia.org/wiki/Scope_(computer_programming) ⟩ для `A`.
- Вне `☷` это правило не действительно. 
В частности, вне `☷` `A` может иметь другое значение вместо `B`.

## Rationale
Подобные local variables помогают мне сокращать запись: я могу в рамках `☷` вводить короткие обозначения, не беспокоясь о конфликте этих обозначений с остальной частью моего запроса.
