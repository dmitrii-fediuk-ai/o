...
It means just `B`.
## Rationale
Я использую `⟨ B ⟩` вместо `B` в том случае, когда `B` — длинный однострочный текст, и я хочу яснее показать его границы (в частности, когда `B` внутри себя содержит спецсимволы).
