...
## Examples
###
`§{2, 3, 5}` означает «`☷` из пунктов `⠿{2, 3, 5}` `᛭Dc`».
###
`A∷§{2, 3, 5}` означает «`☷` из пунктов `⠿{2, 3, 5}` `A`».
...
`☷` — это ⟨ Ⱳ Scope_(computer_programming) ⟩ для `<rules>`.  
...
```
## Meaning
Такой синтаксис используется в связке с синтаксисом §36.
...
# 43. ꘖ
## 43.1. Definitions using a global symbol
### 43.1.1. 
#### Syntax
~~~code
ꘖ A ∋ B
...
## 43.2. Definitions using local keys
### 43.2.1. Common rules
####
В §43.1 мы описывали an attribute / property of `A` using глобально доступный затем символ `B`.
####
В §43.2 мы описываем attributes / properties of `A` иначе: using local keys.
####
...
Конкретный синтаксис для указания контекста описан в §43.2.4. 
