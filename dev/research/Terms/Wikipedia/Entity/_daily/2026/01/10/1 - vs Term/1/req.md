...
~~~
## Meaning
The `ꑌⱳ_Topic` / `ꑌ𝔼` / `ꑌ𐏕` of the `ꑌⱳ_Article` at `https://en.wikipedia.org/wiki/<T>`.  
### Сlarification 
...
~~~
## Meaning
The `ꑌⱳ_Topic` / `ꑌ𝔼` / `ꑌ𐏕`  of the `ꑌⱳ_Article`'s section at `https://en.wikipedia.org/wiki/<T₀>#<T>`. 
### Сlarification 
`<T₀>` — the Last Path Segment of the `ꑌⱲA`'s URL.
...
## Rationale
Я использую этот синтаксис, когда для описываемого мной `ꑌⱳ_Topic` / `ꑌ𝔼` / `ꑌ𐏕` отсутствует a standalone `ꑌⱳ_Article`. 
...
#
`Dꖿ` ≔ ꖿ ⟨ (`ꑌꕤ` of `ꑌ𝔼`) и (`ꑌꕤ` of `ꑌ𐏕` различны) ⟩
...
#
Для каждого довода указывай оценку его значимости (0..100).
