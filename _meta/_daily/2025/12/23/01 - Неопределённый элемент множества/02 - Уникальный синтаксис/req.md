...
# `᛭T`
Придумай мне 50 `AA`: вариантов для выражения `R`.

# `SS`
Спецсимволы из раздела `⦕СИМВОЛЫ⦖` ниже.

# Требования к `AA`
##
Уникальность (решение `P†`).
##
Краткость, выразительность, интуитивная понятность, недвусмысленность.
##
В первую очередь используй `SS`.
##
Во вторую очередь используй аналоги `SS`.
##
Для каждого `AA` укажи обоснование. 
## Не предлагай символы:
⧟ ⊶ 𐏐 ༄ ༆ ꆜ ⫳ ∷ ⫶ ᛡ ⵣ ⁜ ⁝ ⦂ ≙ ⧟ ⊸ ⟜ ⊞ ⌖ ⧇ ⧉
